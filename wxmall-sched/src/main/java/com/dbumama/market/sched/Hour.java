...
public abstract class Hour implements Job {
	private static final Log log = LogFactory.getLog(Hour.class);
    
    private String scheduleId = null;
    private String scheduleName = null;
    private String hour = "1970010100";//已处理的分钟数,默认1970年1月1日0时
    private String minuteSecond = "0000";//处理的秒位数,默认0秒
    ...
    public void process(String datetime)
    {
        ...
        curHour = datetime.substring(0, 10);
        curMinuteSecond = datetime.substring(10);
       
        if ("1970010100".equals(hour) && !isFirstExecute)
        {
            hour = curHour;//直接赋当前小时值，等待下次处理
            return;
        }
        
        if (curHour.compareTo(hour) > 0 && curMinuteSecond.compareTo(minuteSecond) >= 0)
        {
            if (event.isWait())
            {
                event.notifyEvent();
