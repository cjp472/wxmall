...
import com.dbumama.market.service.api.order.ExpTplSaveParamDto;
import com.dbumama.market.service.api.order.OrderException;
import com.dbumama.market.service.api.order.PalletElementResultDto;
import com.dbumama.market.service.base.AbstractServiceImpl;
import com.jfinal.kit.StrKit;

@Service("expressTemplateService")
public class ExpressTemplateServiceImpl extends AbstractServiceImpl implements ExpTemplateService{
	...
	public ExpressTemplate saveTemplate(ExpTplSaveParamDto expTemplateDto) throws OrderException {
		...
		ExpressTemplate etpl = expressTpldao.findFirst("select * from " + ExpressTemplate.table + " where exp_key=? and seller_id=?", expkey, sellerId);
		if(etpl==null){
	...
	public Map<String, List<PalletElementResultDto>> initPalletElement(Long sellerId) {
		...
		elements.add(element);
		/*try {
			String qrcodeUrl = authUserService.getAuthUser().getQrcodeUrl();
	...
	@Override
	public List<ExpressTemplate> getUserExpTemplate(Long sellerId) {
		return expressTpldao.find("select * from " + ExpressTemplate.table + " where seller_id=? and active = 1", sellerId);
	}
}
