...
public class FullCutServiceImpl extends AbstractServiceImpl implements FullCutService{
	...
	public Page<ProductResultDto> getProducts4FullCutPage(ProductParamDto productParamDto) throws ProductException {
		...
		final String sqlExceptSelect = " from "+Product.table+" where seller_id=? and id not in"
	...
	public Page<FullCut> list(PromotionParamDto promotionParam) throws UmpException {
		...
		Page<FullCut> pages = fullCutDao.paginate(promotionParam.getPageNo(), promotionParam.getPageSize(),
				"select * ", 
				" from " + FullCut.table + " where active=1 ");
	...
	@Override
	public FullCut findById(Long id) {
		return fullCutDao.findById(id);
	}
