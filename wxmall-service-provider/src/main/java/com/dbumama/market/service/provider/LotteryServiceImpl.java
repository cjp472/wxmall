...
public class LotteryServiceImpl extends AbstractServiceImpl implements LotteryService{
	...
	private static final LotteryAward lottAwarddao = new LotteryAward().dao();
	private static final LotteryRecord lottRcddao = new LotteryRecord().dao();
	private static final LotteryTrade lottTradedao = new LotteryTrade().dao();
	...
	public Lottery update(Lottery lottery, SellerUser seller, String condition) throws LotteryServiceException {
		...
		if(StrKit.notBlank(condition)){
			if(lottery.getConditionType() == 2){//表示选择的是下单抽奖的条件
				...
				else lottcondTrade = lottTradedao.findById(tradeId); 
				lottcondTrade.setLotteryId(lottery.getId());
				lottcondTrade.setTradeType(jsonObject.getInteger("trade_type"));
				lottcondTrade.setConditionDay(jsonObject.getInteger("condition_day"));
				lottcondTrade.setConditionTradeEnddate(jsonObject.getDate("condition_trade_enddate"));
	...
	public Page<LotteryResultDto> list(int pageNo, int pageSize, Map<String, String> paramsMap) throws LotteryServiceException {
		...
		if(authConfig == null) throw new LotteryServiceException("请配置公众账号");
		...
		Page<Lottery> lotteryPage = lotteryDao.paginate(pageNo, pageSize, 
				"select * ", 
				sqlBuilder.toString(), 
				o);
		...
		for(Lottery lottery : lotteryPage.getList()){
			...
			lvo.setWirlessUrl(authConfig.getWxDomain() + "/lottery/draw/?lotteryId="+lottery.getId());
	...
	public DrawResultDto lottery(Long lotteryId, Long sellerId, Long buyerId) throws LotteryServiceException {
		...
		List<LotteryAward> lawards = lottAwarddao
		...
		//进行行锁，防止并发
		BuyerUser buyer = buyerUserdao.findFirst
	...
	private void checkCondition(Lottery lottery, SellerUser seller, BuyerUser user) throws LotteryServiceException{
		...
		if(lottery.getConditionType() == LotteryCondType.COND_TRADE.value){
			//下单抽奖
			LotteryTrade lottcondTrade = lottTradedao.findFirst
	...
	@Override
	public List<Lottery> getVaildLotterys(Long sellerId) {
		Date currDate = DateTimeUtil.nowDate();
		return lotteryDao.find("select * from " + Lottery.table
	...
	@Override
	public LotteryAward findAwardById(Long id) {
		return lottAwarddao.findById(id);
	}

	@Override
	public List<Record> listLottAwards(Long lotteryId) {
		...
		return Db.find(sql, lotteryId);
	}

	@Override
	public LotteryAward saveAward(LotteryAward award) {
		if(award.getId() == null){
			//check
			List<LotteryAward> awards = lottAwarddao.find("select * from " + LotteryAward.table
					+ " where lottery_id=? ", award.getLotteryId());
			if(awards != null && awards.size() >= 7){
				throw new LotteryServiceException("最多添加7个奖品");
			}
			//对位置进行处理，删除之前位置的奖品
			int pos = award.getAwardPos();
			Long lotteryId = award.getLotteryId();
			LotteryAward aw = lottAwarddao.findFirst(" select * from " + LotteryAward.table
	...
	public void deleteAwardById(Long id) {
		lottAwarddao.deleteById(id);
	}

	@Override
	public List<LotteryAward> initView(Long lotteryId) {
		return lottAwarddao.find("select la.*, pt.display_name as type_name from "
	...
	@Override
	public List<LotteryRecord> getLotteryRecords(Long lotteryId, Long buyerId) {
		return  lottRcddao.find("select * from " + LotteryRecord.table
				+ " where lottery_id=? and buyer_id=? ", lotteryId, buyerId);
	}

	@Override
	public LotteryTrade getLotteryTrade(Long lotteryId) {
		return lottTradedao.findFirst("select * from " + LotteryTrade.table + " where lottery_id = ?", lotteryId);
	}
