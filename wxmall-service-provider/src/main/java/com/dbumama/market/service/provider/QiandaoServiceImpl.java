...
public class QiandaoServiceImpl implements QiandaoService{
	...
	private static final QiandaoConfig qiandaoConfigdao = new QiandaoConfig().dao();
	private static final QiandaoRecord qiandaoRcddao = new QiandaoRecord().dao();
	private static final QiandaoUser qiandaoUserdao = new QiandaoUser().dao();
	...
	public Qiandao doUpdate(Qiandao qiandao, String items, SellerUser sellerUser) 
		...
		for(int i=0;i<jsonArray.size();i++){
			...
			if(StrKit.notBlank(flag) && "update".equals(flag)){
				QiandaoConfig qiandaoItem = qiandaoConfigdao.findById(json.getLong("itemId"));
				if(qiandaoItem != null){
					qiandaoItem.setQiandaoId(qiandao.getId());
					qiandaoItem.set("sign_type", json.getString("signType"));
					qiandaoItem.set("sign_day", json.getString("signDay"));
	...
	public String getCurrQiandao(QiandaoParamDto qiandaoParamDto) throws QiandaoException {
		...
		Qiandao qiandao = qiandaodao.findFirst("select * from " + Qiandao.table + " where seller_id=? " 
		...
		List<QiandaoConfig> qiandaoItems = qiandaoConfigdao.find("select * from " + QiandaoConfig.table 
		...
		//查询粉丝当月签到流水记录
		List<QiandaoRecord> qiandaoRecords = qiandaoRcddao.find("select * from " + QiandaoRecord.table 
		...
		//查询粉丝针对该活动的签到明细记录
		QiandaoUser qiandaoUser = qiandaoUserdao.findFirst("select * from " + QiandaoUser.table 
	...
	public QiandaoSubmitResultDto submit(QiandaoSubmitParamDto qiandaoSubmitParamDto) throws QiandaoException {
		...
		Qiandao qiandao = qiandaodao.findById(qiandaoSubmitParamDto.getQiandaoId());
	...
	private QiandaoSubmitResultDto reward(Qiandao qiandao, QiandaoUser qiandaoUser, QiandaoSubmitParamDto qiandaoSubmitParamDto) throws QiandaoException{
		...
		if(qiandao.getAsSignPrize() == 1){
			qiandaoItems = qiandaoConfigdao.find("select * from " + QiandaoConfig.table 
	...
	@Override
	public Qiandao findById(Long qiandaoId) {
		return qiandaodao.findById(qiandaoId);
	}

	@Override
	public Page<QiandaoResultDto> page(Long sellerId, Integer pageNo, Integer pageSize, String name) {
		
		QueryHelper helper = new QueryHelper("select * ", "from " + Qiandao.table);
		helper.addWhere("seller_id", sellerId).addWhere("active", 1).addWhereLike("qiandao_name", name).addOrderBy("desc", "updated").build();
		
		Page<Qiandao> qiandaoPage = qiandaodao.paginate(pageNo, pageSize, 
				"select * ", 
		...
		for(Qiandao q : qiandaoPage.getList()){
			...
			qiandaoResultDto.setWirlessUrl(authConfig.getWxDomain() + "/qiandao/?qiandaoId="+q.getId());
		...
		return new Page<QiandaoResultDto>(qiandaoResults, qiandaoPage.getPageNumber(), qiandaoPage.getPageSize(), qiandaoPage.getTotalPage(), qiandaoPage.getTotalRow());
	}

	@Override
	public List<QiandaoConfig> getQiandaoConfig(Long qiandaoId) {
		return qiandaoConfigdao.find(" select * from " + QiandaoConfig.table + " where qiandao_id=? and active=1 ", qiandaoId);
	}
