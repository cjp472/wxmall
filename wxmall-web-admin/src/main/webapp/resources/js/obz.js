...
obz.TableView.prototype = {
	...
	query : function(currPage){
		...
		obz.ajaxJson(this.url, this.params, function(resp){
			...
			if(currPage){
				$("#"+me.pageId).pager({table:me, pagenumber:currPage, recordcount:result.totalRow, pagesize:result.pageSize, recordtext:'共 {0} 页, {1} 条记录', buttonClickCallback: me.pageClick });
			}else{
				$("#"+me.pageId).pager({table:me, recordcount:result.totalRow, pagesize:result.pageSize, recordtext:'共 {0} 页, {1} 条记录', buttonClickCallback: me.pageClick });	
			}
...
obz.doSelectImage = function(ImgPath, ImgUrl,ImgId){
	if(obz.selectImageCallBack){
		obz.selectImageCallBack(ImgPath, ImgUrl,ImgId);
	}
...
obz.selectImage = function(params, callBack){
	selectImageDialog = BootstrapDialog.show({
		size: BootstrapDialog.SIZE_WIDE,
		title: "选择图片",
        message: $('<div></div>').load(obz.ctx+'/upload/addImage?model='+params.model)
    });
...
obz.uploadImage = function(params, callBack){
	uploadImageDialog = BootstrapDialog.show({
		...
		title: "上传图片",
        message: $('<div></div>').load(obz.ctx+'/upload/addUploadImage?groupId='+params.groupId)
    });
...
obz.doAddSpecification = function(obj){
	...
	if(addSpecificationDialog) addSpecificationDialog.close();
}
obz.addSpecification = function(params, callBack){
	addSpecificationDialog = BootstrapDialog.show({
		size: BootstrapDialog.SIZE_NORMAL,
		title: "添加规格",
        message: $('<div></div>').load(obz.ctx+'/specification/addSpecification')
    });
...
obz.doAddTouser = function(obj){
	...
	if(addTouserDialog) addTouserDialog.close();
}
