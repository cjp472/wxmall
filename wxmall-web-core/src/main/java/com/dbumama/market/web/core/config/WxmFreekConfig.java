...
public class WxmFreekConfig extends WxmWebConfigAdapter implements WxmFreemarkerConfig{
	...
	public void configFreemarker() {
    	...
    	if(JFinal.me().getConstants().getDevMode()){//不使用缓存
    		cf.setTemplateUpdateDelay(PropKit.getInt("template.update_delay", 0));	
    	}else{
    		cf.setTemplateUpdateDelay(PropKit.getInt("template.update_delay", 300));
    	}
    	...
    	cf.setNumberFormat(PropKit.get("template.number_format"));
    	cf.setBooleanFormat(PropKit.get("template.boolean_format"));
    	cf.setDateTimeFormat(PropKit.get("template.datetime_format"));
    	cf.setDateFormat(PropKit.get("template.date_format"));
    	cf.setTimeFormat(PropKit.get("template.time_format"));
