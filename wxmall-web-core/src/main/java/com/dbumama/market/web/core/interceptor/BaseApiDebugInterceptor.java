 ...
 * @date:2015-10-9
 */
public class BaseApiDebugInterceptor extends IocInterceptor implements Interceptor{
	...
	public void intercept(Invocation ai) {
		...
		if("/system/Interface".equalsIgnoreCase(uri)){
			ai.invoke();
			return;
		}
		
		if("/wxpay/result".equalsIgnoreCase(uri)){
			ai.invoke();
			return;
		}
		if("/alipay/result".equals(uri)){
			ai.invoke();
			return;
		}
		...
		if(StrKit.notBlank(controller.getRequest().getContentType()) 
            ...
            if(uri.equalsIgnoreCase("/diyproduct/save")){
            	if(files!=null&& files.size()>0){
	            	 for(UploadFile f:files){
	            		 String url = "";
	            		 try {
							url = fileToolsService.uploadAttachFile(f.getFile());
						} catch (Exception e) {
							e.printStackTrace();
							continue;
						}
			...
			for(UploadFile file : files){
				String url = "";
				try {
					url = fileToolsService.uploadAttachFile(file.getFile());
				} catch (Exception e) {
					e.printStackTrace();
					continue;
				}
