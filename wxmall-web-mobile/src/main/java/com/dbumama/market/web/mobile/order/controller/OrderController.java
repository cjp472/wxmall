...
public class OrderController extends BaseMobileController{
	...
	public void gcreate(){
		...
		orderParamDto.setMemo(memo);	//买家留言
		try {
			rendSuccessJson(orderService.gcreate(orderParamDto));
		} catch (OrderException e) {
			rendFailedJson(e.getMessage());
		}
	}
	...
	public void joinGroup(){
		final String memo = getPara("memo");
		final String items = getPara("items");
		final Long groupUserId = getParaToLong("group_user_id");//组团发起者
		final OrderJoinParamDto orderJoinParam = new OrderJoinParamDto(getSellerId(), groupUserId, getBuyerId(), getParaToLong("receiverId"), items);
		orderJoinParam.setMemo(memo);
		try {
			Long orderId = orderService.joinGroup(orderJoinParam);
	...
	public void cancel(){
		try {
			orderService.cancel(getParaToLong("orderId"), getBuyerId());
			rendSuccessJson();
		} catch (OrderException e) {
			rendFailedJson(e.getMessage());
		}
	 ...
	 */
	public void detail(){
		OrderResultDto orderDetail = orderService.getOrder(getParaToLong("orderId"), getBuyerId());
