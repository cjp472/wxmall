...
public class PayController extends BaseMobileController{
	...
	public void index(){
		...
		try {
			if(StrKit.notBlank(type)){
				...
				if(rankId != null){
					//会员卡等级购买
					params = cardService.rechargeCardRank(getBuyerId(), cardId, rankId, IpKit.getRealIp(getRequest()));
				}else{
					params = cardService.rechargeCard(getBuyerId(), cardId, getPara("recharge"), IpKit.getRealIp(getRequest()));
				}
			}else if(vip != null && vip==1){
				//vip 处理
				//params = payService.prepareToVipPay(getBuyerId(), getOpenId(), IpKit.getRealIp(getRequest()));
			}else{
	...
	public void gbalance(){
		final String items = getPara("items");
		setAttr("items", items);
		final Long groupUserId = getParaToLong("gHeaderId");
		setAttr("groupUserId", groupUserId);
		//查询默认收货地址
		BuyerReceiver buyerReceiver=receiverService.getDefaultReceiver(getBuyerId());
		if(buyerReceiver != null) {
			Area area=areaService.findById(buyerReceiver.getAreaId());
		...
		try {
			OrderResultDto orderDto = orderService.gbalance(getBuyerId(), null, items);
			setAttr("order", orderDto);
			Card card = cardService.getCardByUser(getBuyerId());
			setAttr("card", card);
